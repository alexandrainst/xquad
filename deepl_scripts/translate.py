...
def translate_xquad_json(xquad_json: dict, target_lang: str, dryrun: bool=False):
    ...
    if dryrun:
        ...
        translate_text = lambda x: translator.translate_text(x, source_lang="en", target_lang=target_lang).text
    else:
        if target_lang == "is":
    ...
    for topic_idx, topic in enumerate(xquad_json["data"]):
        ...
        for paragraph_idx, paragraph in enumerate(topic["paragraphs"]):
            ...
            xquad_translated["data"][topic_idx]["paragraphs"][paragraph_idx]["context"] = translate_text(context)
            for qa_idx, qa in enumerate(paragraph["qas"]):
                question = qa["question"]
                topic_text_len += len(question)
                xquad_translated["data"][topic_idx]["paragraphs"][paragraph_idx]["qas"][qa_idx]["question"] = translate_text(question)
                for ans_idx, answer in enumerate(qa["answers"]):
                    ...
                    xquad_translated["data"][topic_idx]["paragraphs"][paragraph_idx]["qas"][qa_idx]["answers"][ans_idx]["text"] = translate_text(answer_text)
